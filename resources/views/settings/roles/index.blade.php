    ...
    <div class="card shadow-sm border-0 rounded-3">
        <div class="card-header card-premium-header text-white d-flex justify-content-between align-items-center py-3">
            ...
            <div class="d-flex gap-2">
                <button class="btn btn-outline-primary btn-sm fw-bold hover-scale" data-bs-toggle="modal"
        ...
        <div class="card-body p-4">
            ...
            @foreach ($permissionGroups as $groupName => $perms)
                @foreach ($perms as $perm)
                    <form id="delete-perm-{{ $perm['id'] }}" action="{{ route('permissions.destroy', $perm['id']) }}"
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-hover align-middle">
                            ...
                            <tr>
                                @foreach ($roles as $role)
                                    <th>
                                        {{ $role->name }}
                                        <div class="mt-1">
                                            <button type="button" form="delete-role-{{ $role->id }}"
                                                class="btn btn-sm btn-outline-danger py-0 px-1" style="font-size: 0.7rem;"
                                                onclick="if(confirm('Hapus role ini?')) document.getElementById('delete-role-{{ $role->id }}').submit();">Hapus</button>
                                        </div>
                                    </th>
                                @endforeach
                            </tr>
                        ...
                        <tbody>
                            @forelse($permissionGroups as $groupName => $perms)
                                <tr class="table-secondary">
                                    <td colspan="{{ count($roles) + 1 }}"
                                        class="fw-bold text-uppercase fs-7 text-secondary">
                                        {{ $groupName }}
                                    </td>
                                </tr>
                                @foreach ($perms as $perm)
                                            ...
                                            <div class="d-flex justify-content-between align-items-center">
                                                <div>
                                                    ...
                                                    <small class="text-muted d-block"
                                                        style="font-size: 0.7rem;">{{ $perm['name'] }}</small>
                                                </div>
                                                <button type="button" form="delete-perm-{{ $perm['id'] }}"
                                                    class="btn btn-sm text-danger px-1 py-0"
                                                    onclick="if(confirm('Hapus permission ini?')) document.getElementById('delete-perm-{{ $perm['id'] }}').submit();"
                            ...
                            @empty
                                <tr>
                                    <td colspan="{{ count($roles) + 1 }}" class="text-center text-muted py-4">Belum ada
                                        data permission terdaftar.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <input type="text" name="name" class="form-control" required
                                placeholder="Contoh: Kasir, Gudang, Sales">
                        </div>
                    </div>
                ...
                <form action="{{ route('permissions.store') }}" method="POST">
                    ...
                    <div class="modal-body p-4">
                        <div class="mb-3">
                            ...
                            <small class="text-muted mt-1 d-block" style="font-size:0.75rem;">Contoh: <b>view</b>,
                                <b>create</b></small>
                        </div>
                        <div class="mb-3">
                            ...
                            <input type="text" name="menu" class="form-control" required
                                placeholder="Contoh: laporan_sales, mutasi_barang">
                            <small class="text-muted mt-1 d-block" style="font-size:0.75rem;">Jangan gunakan spasi,
                                gunakan underscore (_)</small>
                        </div>
                        <div class="alert alert-light border shadow-sm p-3 mt-3 mb-0" style="font-size:0.85rem;">
                            <i class="fa-solid fa-circle-info text-primary me-1"></i> Permission akan digenerate dengan
                            format: <b>aksi-menu</b> (contoh: <code>view-laporan_sales</code>)
                        </div>
                    </div>
                    <div class="modal-footer bg-light">
                        <button type="button" class="btn btn-outline-secondary fw-semibold"
                            data-bs-dismiss="modal">Batal</button>
                        <button type="submit" class="btn btn-success fw-bold"><i class="fa-solid fa-save me-1"></i>
                            Simpan Permission</button>
                    </div>
                </form>
            </div>
        </div>
