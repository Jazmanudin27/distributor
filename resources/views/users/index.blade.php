...
<div class="card shadow-sm border-0 rounded-3">
    <div class="card-header card-premium-header text-white d-flex justify-content-between align-items-center py-3">
        ...
        <a href="{{ route('users.create') }}" class="btn btn-light btn-sm fw-bold hover-scale">
            <i class="fa-solid fa-circle-plus me-1 text-primary"></i> Tambah User
        </a>
    </div>
                    ...
                    <tr class="hover-row">
                        ...
                        <td>
                            @if($item->jenis_sales === 'kategori')
                                <span class="badge bg-info-subtle text-info border border-info-subtle" title="{{ $item->jenis_barang }}">Kategori: {{ Str::limit($item->jenis_barang, 25) }}</span>
                            @elseif($item->jenis_sales === 'merk')
                                <span class="badge bg-warning-subtle text-warning border border-warning-subtle" title="{{ $item->jenis_barang }}">Merk: {{ Str::limit($item->jenis_barang, 25) }}</span>
                            @else
                                <span class="badge bg-success-subtle text-success border border-success-subtle">Semua Barang</span>
                            @endif
                        </td>
                        ...
                        <td class="text-center">
                            <div class="btn-group gap-1">
                                ...
                                <form action="{{ route('users.destroy', $item->id) }}" method="POST" class="d-inline">
                                    @csrf @method('DELETE')
                                    <button type="button" class="btn btn-sm btn-outline-danger delete rounded" title="Hapus">
