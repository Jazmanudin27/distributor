@extends('layouts.app')
@section('title', $item->exists ? 'Edit Sub Wilayah' : 'Tambah Sub Wilayah')
@section('content')
            ...
            <div class="card shadow-sm border-0 rounded-3">
                <div class="card-header card-premium-header text-white py-3 d-flex justify-content-between align-items-center">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <h5 class="mb-0 fw-bold">{{ $item->exists ? 'Edit Sub Wilayah' : 'Tambah Sub Wilayah Baru' }}</h5>
                            <small class="text-white-50">{{ $item->exists ? 'Perbarui data sub wilayah' : 'Tambahkan data sub wilayah baru' }}</small>
                        </div>
                    </div>
                    <a href="{{ route('sub-wilayah.index') }}" class="btn btn-light btn-sm fw-bold hover-scale">
                        <i class="fa-solid fa-arrow-left me-1 text-primary"></i> Kembali
                    </a>
                </div>

                <div class="card-body p-4">
                    <form action="{{ $item->exists ? route('sub-wilayah.update', $item->kode_wilayah) : route('sub-wilayah.store') }}" method="POST">
                        @csrf
                        @if ($item->exists)
                            @method('PUT')
                        @endif
                        ...
                        <div class="mb-4">
                            <label for="nama_wilayah" class="form-label fs-7 fw-bold text-secondary">Nama Sub Wilayah</label>
                            <div class="input-group input-group-merge input-group-sm">
                                <span class="input-group-text bg-white border-end-0 text-secondary">
                                ...
                                <input type="text" name="nama_wilayah" id="nama_wilayah"
                                    ...
                                    placeholder="Masukkan nama sub wilayah..."
