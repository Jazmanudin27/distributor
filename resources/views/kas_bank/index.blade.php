...
@section('content')
    ...
    <div class="row g-3 mb-4">
        <div class="col-md-4">
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                <div class="card-body p-4 bg-success bg-gradient text-white position-relative">
                    ...
                    <h6 class="text-white-50 text-uppercase fw-semibold mb-1 small" style="font-size: 11px;">Total Debet (Penerimaan)</h6>
        ...
        <div class="col-md-4">
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                <div class="card-body p-4 bg-danger bg-gradient text-white position-relative">
                    ...
                    <h6 class="text-white-50 text-uppercase fw-semibold mb-1 small" style="font-size: 11px;">Total Kredit (Pengeluaran)</h6>
        ...
        <div class="col-md-4">
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                <div class="card-body p-4 {{ $saldo >= 0 ? 'bg-primary' : 'bg-warning' }} bg-gradient text-white position-relative">
                    <div class="position-absolute end-0 top-50 translate-middle-y me-3 opacity-25">
                        <i class="fa-solid fa-scale-balanced fs-1"></i>
                    </div>
                    <h6 class="text-white-50 text-uppercase fw-semibold mb-1 small" style="font-size: 11px;">Saldo Akhir</h6>
            ...
            <div class="bg-light p-3 rounded mb-4 border border-secondary border-opacity-10">
                <form action="{{ route('kas-bank.index') }}" method="GET" class="row g-2 align-items-end">
                    ...
                    <div class="col-md-2 d-flex gap-1">
                        <button type="submit" class="btn btn-sm btn-primary w-100 fw-bold" style="height: 31px;" title="Filter Data">
                            <i class="fa-solid fa-filter me-1"></i> Filter
                        </button>
                        <a href="{{ route('kas-bank.index') }}" class="btn btn-sm btn-outline-secondary d-flex align-items-center justify-content-center" style="width: 40px; height: 31px;" title="Reset">
                        ...
                        @forelse($items as $index => $item)
                            <tr class="hover-row">
                                ...
                                <td class="fw-semibold text-dark">
                                    {{ $item->bank->nama_bank ?? '-' }}
                                    <div class="text-muted small fs-8" style="font-size: 11px;">{{ $item->bank->no_rekening ?? '-' }} - {{ $item->bank->atas_nama ?? '-' }}</div>
                                </td>
                                <td class="text-dark">{{ $item->keterangan ?? '-' }}</td>
                                <td class="text-center">
                                    @if($item->tipe === 'debet')
                                        <span class="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25 px-2 py-1 fs-8">DEBET (Masuk)</span>
                                    @else
                                        <span class="badge bg-danger bg-opacity-10 text-danger border border-danger border-opacity-25 px-2 py-1 fs-8">KREDIT (Keluar)</span>
                                    @endif
                                </td>
                                <td class="text-end fw-bold {{ $item->tipe === 'debet' ? 'text-success' : 'text-danger' }} font-monospace">
