...
<body>
    <div class="container-fluid py-3">
        ...
        <div class="text-center mb-4">
            <h4 class="fw-bold mb-1">LAPORAN PIUTANG & LIMIT KREDIT PELANGGAN ({{ strtoupper($jenis_laporan ?? 'rekap') }})</h4>
            @if($kode_pelanggan)
                <div class="small">Pelanggan: {{ $items->first()['pelanggan']->nama_pelanggan ?? $kode_pelanggan }}</div>
            @endif
                    ...
                    <tbody>
                        ...
                        @foreach ($items as $item)
                            ...
                            <tr>
                                ...
                                <td class="text-end fw-bold text-danger">{{ number_format($item['outstanding'], 0, ',', '.') }}</td>
                                <td class="text-end text-success">{{ number_format($item['sisa_limit'], 0, ',', '.') }}</td>
                                <td class="text-end text-danger">{{ number_format($item['total_overdue'], 0, ',', '.') }}</td>
                    ...
                    <tbody>
                        ...
                        @foreach ($items as $item)
                            ...
                            <tr>
                                ...
                                <td>{{ $item['pelanggan']->nama_pelanggan }}</td>
                                <td>{{ $item['pelanggan']->wilayah->nama_wilayah ?? '-' }}</td>
                                <td class="text-end fw-bold">{{ number_format($item['total_piutang'], 0, ',', '.') }}</td>
                                <td class="text-end text-success">{{ number_format($item['belum_jt'], 0, ',', '.') }}</td>
                                <td class="text-end text-danger">{{ number_format($item['overdue_1_30'], 0, ',', '.') }}</td>
                                <td class="text-end text-danger">{{ number_format($item['overdue_31_60'], 0, ',', '.') }}</td>
                                <td class="text-end text-danger">{{ number_format($item['overdue_61_90'], 0, ',', '.') }}</td>
                                <td class="text-end text-danger">{{ number_format($item['overdue_90'], 0, ',', '.') }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                    <tfoot class="fw-bold">
                    ...
                    <tbody>
                        ...
                        @foreach ($items as $item)
                            ...
                            <tr>
                                ...
                                <td>{{ $item['pelanggan']->nama_pelanggan }} ({{ $item['pelanggan']->kode_pelanggan }})</td>
                                ...
                                <td class="text-end text-success">{{ number_format($item['total_bayar'], 0, ',', '.') }}</td>
                                <td class="text-end fw-bold text-danger">{{ number_format($item['sisa_piutang'], 0, ',', '.') }}</td>
