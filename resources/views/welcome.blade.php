    ...
    <div class="container-fluid p-0">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                ...
                <p class="text-secondary small mb-0">Selamat datang kembali, <strong class="text-white">{{ Auth::user()->name }}</strong>. Berikut adalah kinerja distributor Anda hari ini.</p>
            </div>
        ...
        <div class="row g-4 mb-4">
            ...
            <div class="col-xl-3 col-md-6">
                <div class="card stat-card shadow-sm h-100 p-3 bg-gradient" style="background: linear-gradient(135deg, rgba(26, 29, 39, 0.9) 0%, rgba(16, 185, 129, 0.05) 100%);">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <span class="text-secondary small fw-semibold tracking-wider text-uppercase" style="font-size: 0.7rem;">Penjualan Hari Ini</span>
                            <h3 class="fw-bold text-white mt-2 mb-0">Rp {{ number_format($totalPenjualanHariIni, 0, ',', '.') }}</h3>
                        </div>
            ...
            <!-- Card 3: Pembelian Hari Ini -->
            <div class="col-xl-3 col-md-6">
                <div class="card stat-card shadow-sm h-100 p-3" style="background: linear-gradient(135deg, rgba(26, 29, 39, 0.9) 0%, rgba(245, 158, 11, 0.05) 100%);">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <span class="text-secondary small fw-semibold tracking-wider text-uppercase" style="font-size: 0.7rem;">Pembelian Hari Ini</span>
                            <h3 class="fw-bold text-white mt-2 mb-0">Rp {{ number_format($totalPembelianHariIni, 0, ',', '.') }}</h3>
                        </div>
            ...
            <!-- Card 4: Piutang Outstanding -->
            <div class="col-xl-3 col-md-6">
                <div class="card stat-card shadow-sm h-100 p-3" style="background: linear-gradient(135deg, rgba(26, 29, 39, 0.9) 0%, rgba(239, 68, 68, 0.05) 100%);">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <span class="text-secondary small fw-semibold tracking-wider text-uppercase" style="font-size: 0.7rem;">Total Outstanding Piutang</span>
            ...
            <div class="col-xl-4 col-lg-6">
                <div class="card list-card shadow-sm">
                    <div class="card-header bg-transparent py-3 d-flex justify-content-between align-items-center">
                        ...
                        <span class="badge bg-success-subtle text-success badge-pulse">{{ $activeCheckins->count() }} Online</span>
                    </div>
                                        ...
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div class="flex-grow-1">
                                                <div class="d-flex justify-content-between">
                                                    <span class="fw-bold text-white small">{{ $checkin->sales->name ?? 'Sales' }}</span>
                                                ...
                                                <p class="mb-0 text-secondary" style="font-size: 0.75rem;">
                                                    Kunjungan: <strong class="text-light-indigo">{{ $checkin->pelanggan->nama_pelanggan ?? 'Toko' }}</strong>
                                                </p>
                                                @if($checkin->catatan)
                                                    <span class="text-muted italic" style="font-size: 0.7rem;">"{{ Str::limit($checkin->catatan, 35) }}"</span>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                        ...
                        @else
                            <div class="list-group list-group-flush">
                                @foreach($lowStockItems as $item)
                                    ...
                                    @endphp
                                    <div class="list-group-item bg-transparent border-white-10 py-3 px-4 d-flex justify-content-between align-items-center">
                                        <div>
                                            <span class="fw-semibold text-white small d-block">{{ $item->nama_barang }}</span>
                                            <small class="text-secondary">{{ $item->kode_barang }}</small>
                                        </div>
                                        <div class="text-end">
                                            <span class="badge bg-danger text-white fw-bold">{{ $item->stok }} {{ $satuanName }}</span>
                                            <small class="d-block text-secondary small" style="font-size: 0.7rem;">Kritis</small>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                ...
                <div class="card list-card shadow-sm">
                    <div class="card-header bg-transparent py-3 d-flex justify-content-between align-items-center">
                        ...
                        @if($pendingApprovalKredit > 0)
                            <span class="badge bg-warning-subtle text-warning">{{ $pendingApprovalKredit }} Butuh Approval</span>
                        @endif
                    </div>
                    <div class="card-body p-0" style="max-height: 320px; overflow-y: auto;">
                                ...
                                @foreach($latestLimitRequests as $req)
                                    <div class="list-group-item bg-transparent border-white-10 py-3 px-4">
                                        <div class="d-flex justify-content-between align-items-start">
                                            <div>
                                                <span class="fw-semibold text-white small d-block">{{ $req->pelanggan->nama_pelanggan ?? 'Toko' }}</span>
                                                <small class="text-secondary">Diajukan oleh: {{ $req->requester->name ?? 'Sales' }}</small>
                                                <div class="mt-1" style="font-size: 0.75rem;">
                                                    <span class="text-secondary">Rp {{ number_format($req->limit_lama, 0, ',', '.') }}</span>
                                                    <i class="fa-solid fa-arrow-right mx-1 text-muted"></i>
                                                    <strong class="text-success">Rp {{ number_format($req->limit_baru, 0, ',', '.') }}</strong>
                                                </div>
                                            </div>
                                            <span class="badge bg-{{ $req->status_color }}-subtle text-{{ $req->status_color }} fs-8">
                                                {{ $req->status_label }}
                                            </span>
                                        </div>
                                    </div>
                            ...
                            tooltip: {
                                callbacks: {
                                    label: function(context) {
                                        ...
                                        return ' Penjualan: Rp ' + Number(value).toLocaleString('id-ID');
                                    }
                                }
                            }
                        },
