        ...
        <div class="col-lg-10 col-md-12">
            <div class="card shadow-sm border-0 rounded-3">
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <h5 class="mb-0 fw-bold">
                                {{ $item->exists ? 'Edit Aturan Diskon Strata' : 'Tambah Diskon Strata Baru' }}</h5>
                            <small
                                class="text-white-50">{{ $item->exists ? 'Perbarui kriteria dan tingkatan/strata diskon' : 'Buat aturan diskon berjenjang baru' }}</small>
                        </div>
                    </div>
                ...
                <div class="card-body p-4 bg-light">
                    <form
                        action="{{ $item->exists ? route('diskon-strata.update', $item->id) : route('diskon-strata.store') }}"
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                <div class="card p-3 border rounded bg-white h-100 mb-0 shadow-sm">
                                    ...
                                    <div class="mb-2">
                                        <label for="nama_diskon" class="form-label fs-7 fw-bold text-secondary">Nama
                                            Promo/Diskon <span class="text-danger">*</span></label>
                                    ...
                                    <div class="mb-2">
                                        <label for="tipe" class="form-label fs-7 fw-bold text-secondary">Tipe Aturan
                                        ...
                                        <select name="tipe" id="tipe" class="form-select form-select-sm" required
                                            ...
                                            <option value="barang"
                                                {{ old('tipe', $item->tipe) === 'barang' ? 'selected' : '' }}>Per Barang
                                            </option>
                                            <option value="beberapa_barang"
                                                {{ old('tipe', $item->tipe) === 'beberapa_barang' ? 'selected' : '' }}>Per
                                                Beberapa Barang</option>
                                            <option value="kategori"
                                                {{ old('tipe', $item->tipe) === 'kategori' ? 'selected' : '' }}>Per
                                                Kategori</option>
                                            <option value="merk"
                                                {{ old('tipe', $item->tipe) === 'merk' ? 'selected' : '' }}>Per Merk
                                            </option>
                                            <option value="supplier"
                                                {{ old('tipe', $item->tipe) === 'supplier' ? 'selected' : '' }}>Per
                                                Supplier (Nominal)</option>
                                        </select>
                                        @if ($item->exists)
                                    ...
                                    <div class="mb-0">
                                        <div class="form-check form-switch mt-3">
                                            ...
                                            <label class="form-check-label fs-7 fw-bold text-secondary"
                                                for="is_active">Aktifkan Aturan Diskon</label>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="card p-3 border rounded bg-white h-100 mb-0 shadow-sm">
                                    ...
                                    <div id="target_barang_wrapper" class="target-section mb-2 d-none">
                                        <label for="barang_ids" class="form-label fs-7 fw-bold text-secondary"
                                            id="label_barang_ids">Pilih Barang <span class="text-danger">*</span></label>
                                        <select id="barang_ids" name="barang_ids[]"
                                            class="form-select form-select-sm select2-multiple" style="width: 100%;"
                                            multiple>
                                    ...
                                    <div id="target_kategori_wrapper" class="target-section mb-2 d-none">
                                        <label for="kategori_id" class="form-label fs-7 fw-bold text-secondary">Pilih
                                    ...
                                    <div id="target_merk_wrapper" class="target-section mb-2 d-none">
                                        <label for="merk_id" class="form-label fs-7 fw-bold text-secondary">Pilih Merk
                                            <span class="text-danger">*</span></label>
                                        <select name="merk_id" id="merk_id" class="form-select form-select-sm">
                                            <option value="">-- Pilih Merk --</option>
                                            @foreach ($merks as $m)
                                                <option value="{{ $m->id }}"
                                                    {{ old('merk_id', $item->merk_id) == $m->id ? 'selected' : '' }}>
                                                    {{ $m->nama_merk }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div id="target_supplier_wrapper" class="target-section mb-2 d-none">
                                        <label for="kode_supplier" class="form-label fs-7 fw-bold text-secondary">Pilih
                                    ...
                                    <div class="row g-2">
                                        <div class="col-6">
                                            <label for="berlaku_dari" class="form-label fs-7 fw-bold text-secondary">Mulai
                                                Berlaku</label>
                                            <input type="datetime-local" name="berlaku_dari" id="berlaku_dari"
                                                class="form-control form-control-sm"
                                                value="{{ old('berlaku_dari', $item->berlaku_dari ? $item->berlaku_dari->format('Y-m-d\TH:i') : '') }}">
                                        </div>
                                        <div class="col-6">
                                            <label for="berlaku_sampai"
                                                class="form-label fs-7 fw-bold text-secondary">Selesai Berlaku</label>
                                            <input type="datetime-local" name="berlaku_sampai" id="berlaku_sampai"
                                                class="form-control form-control-sm"
        ...
        $(document).ready(function() {
            ...
            $('#tipe').on('change', function() {
                ...
                if (type === 'barang' || type === 'beberapa_barang') {
                    ...
                    if (type === 'barang') {
                        ...
                        $('#label_barang_ids').html(
                            'Pilih Beberapa Barang <span class="text-danger">*</span>');
            ...
            $(document).on('click', '.btn-remove-tier', function() {
                if ($('#tiersTable tbody tr').length > 1) {
                    ...
                    Swal.fire('Informasi', 'Aturan diskon minimal harus memiliki 1 tingkatan (strata)!',
                        'info');
                }
            });
            ...
            $('#diskonStrataForm').on('submit', function(e) {
                ...
                if (!isValid) {
                    e.preventDefault();
                    return Swal.fire('Error Validasi',
                        'Batas Maksimum strata tidak boleh lebih kecil dari batas Minimum!', 'error');
                }
            });
        });
    </script>
