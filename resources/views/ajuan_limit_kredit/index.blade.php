    ...
    <div class="card shadow-sm border-0 rounded-3">
        <div class="card-header card-premium-header text-white d-flex justify-content-between align-items-center py-3">
            ...
            @can('create-ajuan_limit_kredit')
                <a href="{{ route('ajuan-limit-kredit.create') }}" class="btn btn-light btn-sm fw-bold hover-scale">
                    <i class="fa-solid fa-circle-plus me-1 text-primary"></i> Buat Ajuan
                </a>
            @endcan
        </div>
        ...
        <div class="card-body p-4">
            ...
            <div class="row g-3 mb-4">
                <div class="col-6 col-md-3">
                    <div class="card stat-card shadow-sm h-100" style="background: linear-gradient(135deg,#f8faff,#eef2ff);">
                ...
                <div class="col-6 col-md-3">
                    <div class="card stat-card shadow-sm h-100" style="background: linear-gradient(135deg,#fffbeb,#fef9c3);">
                        <div class="card-body d-flex align-items-center gap-3 py-3">
                ...
                <div class="col-6 col-md-3">
                    <div class="card stat-card shadow-sm h-100" style="background: linear-gradient(135deg,#f0fdf4,#dcfce7);">
                        <div class="card-body d-flex align-items-center gap-3 py-3">
                ...
                <div class="col-6 col-md-3">
                    <div class="card stat-card shadow-sm h-100" style="background: linear-gradient(135deg,#fff1f2,#ffe4e6);">
                        <div class="card-body d-flex align-items-center gap-3 py-3">
            ...
            <div class="d-flex flex-wrap gap-2 align-items-center mb-3">
                <form action="{{ route('ajuan-limit-kredit.index') }}" method="GET" class="d-flex gap-2 flex-wrap align-items-center w-100">
                    <div class="d-flex gap-1 flex-wrap">
                        <button type="submit" name="status" value="" class="filter-tab {{ !request('status') ? 'active' : '' }}">
                            Semua
                        </button>
                        <button type="submit" name="status" value="pending" class="filter-tab {{ request('status') === 'pending' ? 'active' : '' }}">
                            <i class="fa-solid fa-hourglass-half me-1"></i> Pending
                            @if(isset($statusCounts['pending']) && $statusCounts['pending'] > 0)
                                <span class="badge bg-warning text-dark rounded-pill ms-1" style="font-size: 0.7rem; padding: 0.2em 0.5em;">{{ $statusCounts['pending'] }}</span>
                            @endif
                        </button>
                        <button type="submit" name="status" value="approved" class="filter-tab {{ request('status') === 'approved' ? 'active' : '' }}">
                            <i class="fa-solid fa-check me-1"></i> Disetujui
                        </button>
                        <button type="submit" name="status" value="rejected" class="filter-tab {{ request('status') === 'rejected' ? 'active' : '' }}">
                    ...
                    <tbody>
                        @forelse($ajuans as $index => $ajuan)
                            <tr class="table-row-hover">
                                ...
                                <td class="text-end fw-semibold text-{{ $ajuan->limit_baru > $ajuan->limit_lama ? 'success' : ($ajuan->limit_baru < $ajuan->limit_lama ? 'danger' : 'secondary') }}">
                                ...
                                <td class="text-center">
                                    <span class="badge-status badge bg-{{ $ajuan->status_color }}-subtle text-{{ $ajuan->status_color }} border border-{{ $ajuan->status_color }}-subtle">
