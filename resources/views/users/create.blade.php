                ...
                <form action="{{ route('users.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="jenis_sales" id="jenis_sales" class="form-select form-select-sm" onchange="toggleRestrictionFields()">
                            <option value="semua" {{ old('jenis_sales') == 'semua' ? 'selected' : '' }}>Semua Barang (Tanpa Batasan)</option>
                            <option value="kategori" {{ old('jenis_sales') == 'kategori' ? 'selected' : '' }}>Per Kategori</option>
                            <option value="merk" {{ old('jenis_sales') == 'merk' ? 'selected' : '' }}>Per Merk</option>
                        </select>
                    </div>
                    ...
                    <div class="mb-3 d-none" id="kategori-container">
                        ...
                        <select name="jenis_barang[]" id="jenis_barang_kategori" class="form-select form-select-sm" multiple style="height: 120px;">
                            @foreach($kategoris as $k)
                                <option value="{{ $k->nama_kategori }}" {{ is_array(old('jenis_barang')) && in_array($k->nama_kategori, old('jenis_barang')) ? 'selected' : '' }}>
                                    {{ $k->nama_kategori }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3 d-none" id="merk-container">
                        ...
                        <select name="jenis_barang[]" id="jenis_barang_merk" class="form-select form-select-sm" multiple style="height: 120px;">
                            @foreach($merks as $m)
                                <option value="{{ $m->nama_merk }}" {{ is_array(old('jenis_barang')) && in_array($m->nama_merk, old('jenis_barang')) ? 'selected' : '' }}>
                                    {{ $m->nama_merk }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3 form-check ms-1">
                        <input type="checkbox" name="is_kanvas" id="is_kanvas" class="form-check-input" value="1" {{ old('is_kanvas') ? 'checked' : '' }}>
