            ...
            @foreach ($pendingCustomers as $p)
                <div class="mobile-card p-3 mb-0">
                    ...
                    <div class="d-flex gap-2 border-top border-secondary border-opacity-10 pt-3">
                        <form action="{{ route('mobile.spv.pelanggan.reject', $p->kode_pelanggan) }}" method="POST" class="flex-grow-1" onsubmit="return confirm('Apakah Anda yakin ingin menolak pelanggan ini?')">
                            @csrf
                        ...
                        <form action="{{ route('mobile.spv.pelanggan.approve', $p->kode_pelanggan) }}" method="POST" class="flex-grow-1" onsubmit="return confirm('Apakah Anda yakin ingin menyetujui pelanggan ini?')">
                            @csrf
