...
@section('content')
    <div class="card shadow-sm border-0 rounded-3">
        ...
        <div class="card-body p-4">
            {{-- Salesman Selection --}}
            <form method="GET" action="{{ route('canvas.returns.create') }}" class="mb-4" id="select-sales-form">
                <div class="row align-items-end g-2">
                    <div class="col-md-4">
                        <label class="form-label text-secondary fw-semibold mb-1">Pilih Salesman</label>
                        <select name="kode_sales" class="form-select form-select-sm" onchange="this.form.submit()">
                            <option value="">— Pilih Salesman —</option>
                            @foreach ($salesmen as $s)
                                <option value="{{ $s->nik }}" {{ $selectedSales == $s->nik ? 'selected' : '' }}>
                                    {{ $s->name }} (NIK: {{ $s->nik }})
                                </option>
                            @endforeach
                        </select>
            ...
            @if ($selectedSales)
                ...
                @else
                    ...
                    <div class="alert alert-warning border-warning rounded-3 mb-4 d-flex align-items-start" style="background-color: rgba(245, 158, 11, 0.08);">
                        <i class="fa-solid fa-triangle-exclamation fs-4 text-warning me-3 mt-1"></i>
                        <div>
                            <h6 class="alert-heading fw-bold mb-1" style="color: #d97706;">Akumulasi {{ $activeSessions->count() }} DPB Aktif</h6>
                            <p class="mb-0 small text-dark">
                                Kuantitas pengambilan dan penjualan di bawah ini merupakan akumulasi dari seluruh DPB aktif berikut:
                    ...
                    <form action="{{ route('canvas.returns.store') }}" method="POST">
                        @csrf
                            ...
                            <tbody>
                                ...
                                @foreach ($accumulatedDetails as $key => $item)
                                    @php
                                        ...
                                        $expectedSisa = max(0.0, (float) $item['qty_ambil'] - (float) $item['qty_terjual']);
                                        $qtyAmbilSmallest = $item['qty_ambil'] * ($item['barangSatuan']->isi ?? 1);
                                        $qtyTerjualSmallest = $item['qty_terjual'] * ($item['barangSatuan']->isi ?? 1);
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <span class="text-secondary small font-11">Kode: {{ $item['kode_barang'] }}</span>
                                        </td>
                                        ...
                                        <td class="text-end pe-3 bg-primary-subtle text-primary fw-bold">
                                            <div class="fs-7">{{ $item['barang']->formatStok($qtyAmbilSmallest) }}</div>
                                            <small class="text-secondary font-11 d-block">({{ (float) $item['qty_ambil'] }} {{ $item['barangSatuan']->satuan ?? 'PCS' }})</small>
                                        </td>
                                        <td class="text-end pe-3 bg-info-subtle text-info fw-bold">
                                            <div class="fs-7">{{ $item['barang']->formatStok($qtyTerjualSmallest) }}</div>
                                            <small class="text-secondary font-11 d-block">({{ (float) $item['qty_terjual'] }} {{ $item['barangSatuan']->satuan ?? 'PCS' }})</small>
                                        </td>
                                        <td class="bg-success-subtle px-3">
                                            <input type="hidden" name="details[{{ $index }}][detail_ids]" value="{{ $detailIds }}">
                                                ...
                                                <div class="d-flex flex-wrap gap-1 justify-content-center">
                                                    ...
                                                    @else
                                                        <div class="input-group input-group-sm" style="width: 100px;">
                                                            <input type="number"
                                                                class="form-control text-center input-unit-qty input-qty-row-{{ $index }}"
                                                                data-isi="1" data-id="0" data-row-id="{{ $index }}"
                                                                value="{{ $expectedSisa }}" min="0" step="any">
                                        ...
                                        <td class="text-end pe-3 fw-bold">
                                            <div class="fs-7 text-dark">{{ $item['barang']->formatStok($qtyExpectedSmallest) }}</div>
                                            <small class="text-secondary font-11 d-block">({{ (float) $expectedSisa }} {{ $item['barangSatuan']->satuan ?? 'PCS' }})</small>
                                        </td>
                                    </tr>
                                    @php $index++; @endphp
                                @endforeach
