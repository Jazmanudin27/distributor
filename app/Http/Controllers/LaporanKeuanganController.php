...
class LaporanKeuanganController extends Controller
{
    ...
    public function laporanRekapSisaPiutang(Request $request)
    {
        ...
        if ($isPrintOrExcel) {
            ...
            if ($wilayah_id) {
                $pelangganByWilayah = \App\Models\Pelanggan::where('kode_wilayah', $wilayah_id)->pluck('kode_pelanggan');
                $query->whereIn('kode_pelanggan', $pelangganByWilayah);
            }
            if ($sub_wilayah_id) {
                $pelangganBySubWilayah = \App\Models\Pelanggan::where('sub_wilayah', $sub_wilayah_id)->pluck('kode_pelanggan');
                $query->whereIn('kode_pelanggan', $pelangganBySubWilayah);
            }
    ...
    public function laporanSetoran(Request $request)
    {
        ...
        if ($isExcel) {
            ...
            return response(view($view, compact('salesmen', 'items', 'tanggal_mulai', 'tanggal_akhir', 'kode_sales', 'jenis_bayar', 'isExcel', 'kategoriSales')))
            ->header('Content-Type', 'application/vnd-ms-excel')
            ->header('Content-Disposition', 'attachment; filename="' . $filename . '"')
            ->header('Pragma', 'no-cache')
            ->header('Expires', '0');
        ...
        return view($view, compact('salesmen', 'items', 'tanggal_mulai', 'tanggal_akhir', 'kode_sales', 'jenis_bayar', 'kategoriSales'));
    }
