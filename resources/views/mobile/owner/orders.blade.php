...
@push('styles')
    <style>
        .select2-mobile + .select2-container .select2-selection--single {
            background-color: #121824 !important;
            border: 1px solid var(--border-color) !important;
            border-radius: 12px !important;
            height: 44px !important;
        }
        .select2-mobile + .select2-container .select2-selection--single .select2-selection__rendered {
            color: var(--text-primary) !important;
            line-height: 42px !important;
        }
        .select2-mobile + .select2-container .select2-selection--single .select2-selection__arrow {
...
@section('content')
    ...
    <div class="mobile-card p-3 mb-3">
        <div class="d-flex justify-content-between align-items-center" style="cursor: pointer;" data-bs-toggle="collapse" data-bs-target="#filterCollapse" aria-expanded="false" aria-controls="filterCollapse">
            <h6 class="fw-bold mb-0 text-white" style="font-size: 0.85rem;"><i class="fa-solid fa-sliders me-1.5 text-primary"></i> Filter Pencarian</h6>
        ...
        <div class="collapse" id="filterCollapse">
            <form action="{{ route('mobile.owner.order.index') }}" method="GET" class="mt-3">
                ...
                <div class="mb-2">
                    <label class="form-label text-secondary small fw-semibold mb-1">Cari No. Faktur / Pelanggan</label>
                    <input type="text" name="q" value="{{ $q }}" class="form-control form-control-mobile py-2" placeholder="Ketik kata kunci...">
                </div>

                <div class="mb-2">
                    <label class="form-label text-secondary small fw-semibold mb-1">Filter Sales</label>
                    <select name="kode_sales" class="form-select form-control-mobile py-2 select2-mobile">
                ...
                <div class="mb-3">
                    <label class="form-label text-secondary small fw-semibold mb-1">Filter Pelanggan</label>
                    <select name="kode_pelanggan" class="form-select form-control-mobile py-2 select2-mobile">
                        <option value="">-- Semua Pelanggan --</option>
                        @foreach($pelanggans as $p)
                            <option value="{{ $p->kode_pelanggan }}" {{ $selected_pelanggan == $p->kode_pelanggan ? 'selected' : '' }}>
                ...
                <div class="d-flex gap-2">
                    <a href="{{ route('mobile.owner.order.index') }}" class="btn btn-mobile btn-outline-secondary w-50 py-2 fw-semibold fs-8.5 border-secondary text-secondary">Reset</a>
    ...
    <div class="d-flex gap-2 mb-3 pb-1" style="overflow-x: auto; -webkit-overflow-scrolling: touch;">
        <a href="{{ route('mobile.owner.order.index', ['filter' => 'all', 'q' => $q, 'kode_sales' => $selected_sales, 'kode_pelanggan' => $selected_pelanggan]) }}" 
        ...
        <a href="{{ route('mobile.owner.order.index', ['filter' => 'today', 'q' => $q, 'kode_sales' => $selected_sales, 'kode_pelanggan' => $selected_pelanggan]) }}" 
        ...
        <a href="{{ route('mobile.owner.order.index', ['filter' => 'month', 'q' => $q, 'kode_sales' => $selected_sales, 'kode_pelanggan' => $selected_pelanggan]) }}" 
                    ...
                    <div class="d-flex justify-content-between align-items-start mb-2 border-bottom border-secondary border-opacity-10 pb-2">
                        ...
                        <div class="text-end">
                            <div class="mb-1 d-flex justify-content-end gap-1 flex-wrap">
                                ...
                                @else
                                    <span class="badge {{ $order->jenis_transaksi === 'Tunai' ? 'bg-success-subtle text-success' : 'bg-primary-subtle text-primary' }} px-2 py-1"
                ...
                <div class="collapse" id="details-{{ str_replace('-', '_', $order->no_faktur) }}">
                    <div class="p-3 rounded-4 mb-3"
                        ...
                        <div class="pt-2 mt-1" style="font-size: 0.75rem;">
                            ...
                            @endif
                            <div class="d-flex justify-content-between text-secondary mb-1 pt-1 border-top border-secondary border-opacity-10 mt-1">
                                <span>Total Tagihan:</span>
                                <span class="text-white fw-bold">Rp {{ number_format($order->grand_total, 0, ',', '.') }}</span>
                            </div>
                            <div class="d-flex justify-content-between text-secondary mb-1">
                            ...
                            <div class="d-flex justify-content-between text-secondary mb-1">
                                ...
                                <span class="{{ $sisaBayar < 1 ? 'text-success fw-semibold' : 'text-warning fw-semibold' }}">
                            ...
                            @if (in_array($order->jenis_transaksi, ['K', 'Kredit']))
                                <div class="d-flex justify-content-between text-secondary mb-1">
                                    ...
                                    <span class="{{ $isOverdue ? 'text-danger fw-semibold' : 'text-info fw-semibold' }}">
                                        ...
                                        @if ($sisaBayar >= 1)
                                            @if ($isOverdue)
                                                (Overdue {{ (int) round(\Carbon\Carbon::today()->diffInDays($dueDate)) }} Hari)
                                            @else
                                                (Sisa {{ (int) round(\Carbon\Carbon::today()->diffInDays($dueDate, false) * -1) }} Hari)
                                            @endif
                                        @endif
                                    </span>
                                </div>
                                            ...
                                            <div>
                                                ...
                                                @if ($bayar->keterangan)
                                                    <small class="d-block text-muted mt-0.5"
                                                        style="font-size: 0.65rem;">Ket: "{{ $bayar->keterangan }}"</small>
                                                @endif
                                            </div>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            @if($q || $selected_sales || $selected_pelanggan)
